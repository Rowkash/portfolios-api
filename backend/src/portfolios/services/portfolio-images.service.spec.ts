...
describe('PortfolioImagesService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getModelToken(PortfolioImageModel),
          useValue: {},
        },
  ...
  describe('getIncludes()', () => {
    it.each`
      payload                | expectedResult
      ${{}}                  | ${{}}
      ${{ portfolio: true }} | ${{ model: PortfolioModel }}
      ${{ portfolio: 223 }}  | ${{}}
    `(
      'should return correct filter $payload',
      ({ payload, expectedResult }) => {
        const result = portfolioImagesService.getIncludes(
          payload as IGetPortfolioImageIncludesOptions,
        );
        expect(result).toEqual(expectedResult);
      },
    );
